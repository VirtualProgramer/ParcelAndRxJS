...
},{"../internal/observable/dom/ajax":"node_modules/rxjs/_esm5/internal/observable/dom/ajax.js","../internal/observable/dom/AjaxObservable":"node_modules/rxjs/_esm5/internal/observable/dom/AjaxObservable.js"}],"components/12. operators.ts":[function(require,module,exports) {
"use strict";
...
var ajax_1 = require("rxjs/ajax"); //----- 1. EMPTY：空的 Observable，沒有任何事件，就直接結束了


rxjs_1.EMPTY.subscribe(function (data) {
  return function (data) {
    return console.log("empty \u7BC4\u4F8B: " + data);
  };
}); // (不會印出任何東西)
...
rxjs_1.EMPTY.subscribe({
  ...
  complete: function complete() {
    return console.log('empty 結束');
  }
}); // empty 結束
//----- 2. of：將傳進去的值當作一條 Observable，當值都發送完後結束

rxjs_1.of(1).subscribe(function (data) {
  return console.log("of \u7BC4\u4F8B: " + data);
}); // of 範例: 1

rxjs_1.of(1, 2, 3, 4).subscribe(function (data) {
  return console.log("of \u7BC4\u4F8B: " + data);
}); // of 範例: 1
// of 範例: 2
// of 範例: 3
...
rxjs_1.range(3, 4).subscribe(function (data) {
  return console.log("range \u7BC4\u4F8B: " + data);
}); // range 範例: 3
// range 範例: 4
// range 範例: 5
// range 範例: 6
...
var emitHelloIfEven = function emitHelloIfEven(data) {
  return rxjs_1.iif(function () {
    return data % 2 === 0;
  }, rxjs_1.of('Hello'), rxjs_1.EMPTY);
};

emitHelloIfEven(1).subscribe(function (data) {
  return console.log("iif \u7BC4\u4F8B (1): " + data);
}); // (不會印出任何東西)

emitHelloIfEven(2).subscribe(function (data) {
  return console.log("iif \u7BC4\u4F8B (2): " + data);
}); // iif 範例 (2): Hello
//----- 5. throwError：透過條件來決定產生怎麼樣的 Observable，有三個參數

var rxjs_2 = require("rxjs");

var source$ = rxjs_2.throwError('發生錯誤了');
source$.subscribe({
  next: function next(data) {
    return console.log("throwError \u7BC4\u4F8B (next): " + data);
  },
  error: function error(_error) {
    return console.log("throwError \u7BC4\u4F8B (error): " + _error);
  },
  complete: function complete() {
    return console.log('throwError 範例 (complete)');
  }
}); // throwError 範例 (error): 發生錯誤了
//----- 6. ajax：用來發送 HTTP 請求抓 API 資料的，會回傳 ajaxResponse 格式

var source2$ = ajax_1.ajax('https://api.github.com/repos/reactivex/rxjs/issues');
source2$.subscribe(function (result) {
  return console.log(result.response);
}); // 把 fetch API 包裝成 Observable 的範例程式

var source3$ = new rxjs_1.Observable(function (subscriber) {
  fetch('https://api.github.com/repos/reactivex/rxjs/issues').then(function (response) {
...
rxjs_1.from(createRange(1, 4)).subscribe(function (data) {
...
rxjs_1.from(Promise.resolve(1)).subscribe(function (data) {
  console.log("from \u793A\u7BC4 (3): " + data);
}); // from 示範 (3): 1
// 傳遞 Observable 當參數

rxjs_1.from(rxjs_1.of(1, 2, 3, 4)).subscribe(function (data) {
...
rxjs_1.fromEvent(document, 'click').subscribe(function (data) {
...
var addClickHandler = function addClickHandler(handler) {
...
var removeClickHandler = function removeClickHandler(handler) {
  console.log('fromEventPattern 示範: 自定義取消滑鼠事件');
  document.removeEventListener('click', handler);
};

var source4$ = rxjs_1.fromEventPattern(addClickHandler, removeClickHandler);
var subscription = source4$.subscribe(function (event) {
  return console.log('fromEventPattern 示範: 滑鼠事件發生了', event);
});
setTimeout(function () {
  subscription.unsubscribe();
}, 3000); //----- 10. interval：依照參數設定的時間 (毫秒) 來建議 Observable，當被訂閱時，就會每隔一段指定的時間發生一次資料流

rxjs_1.interval(1000).subscribe(function (data) {
  return console.log("interval \u793A\u7BC4: " + data);
}); // 可以在一段時間後把它取消訂閱來結束 Observable：

var subscription2 = rxjs_1.interval(1000).subscribe(function (data) {
  return console.log("interval \u793A\u7BC4: " + data);
});
setTimeout(function () {
  subscription2.unsubscribe();
}, 5500); //----- 11. interval：跟 interval 有點類似，但它多一個參數，用來設定經過多久時間後開始依照指定的間隔時間計時

rxjs_1.timer(3000, 1000).subscribe(function (data) {
  return console.log("timer \u793A\u7BC4 (1): " + data);
}); // timer 示範 (1): 0
// timer 示範 (1): 1
// timer 示範 (1): 2
// timer 示範 (1): 3
...
rxjs_1.timer(3000).subscribe(function (data) {
  console.log("timer \u793A\u7BC4 (2): " + data);
}); // timer 示範 (2): 0
},{"rxjs":"node_modules/rxjs/_esm5/index.js","rxjs/ajax":"node_modules/rxjs/_esm5/ajax/index.js"}],"C:/Users/Jin-Wen/AppData/Roaming/npm/node_modules/parcel-bundler/src/builtins/hmr-runtime.js":[function(require,module,exports) {
var global = arguments[3];
...
},{}]},{},["C:/Users/Jin-Wen/AppData/Roaming/npm/node_modules/parcel-bundler/src/builtins/hmr-runtime.js","components/12. operators.ts"], null)
//# sourceMappingURL=/12.%20operators.759a1e32.js.map